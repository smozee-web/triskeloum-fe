...
const ContactSection = () => {
  ...
  const title = contactSection ? localized(contactSection, 'title', lang) : t('Commencez votre voyage', 'Begin Your Journey', 'ابدأ رحلتك');
  const subtitle = contactSection ? localized(contactSection, 'subtitle', lang) : t('CONTACT', 'CONTACT', 'تواصل معنا');
  const description = contactSection ? localized(contactSection, 'description', lang) : t("Prêt à commencer votre purification et votre guérison ? Contactez le Cheikh directement sur WhatsApp pour une première consultation.", "Ready to begin your purification and healing? Contact the Sheikh directly on WhatsApp for an initial consultation.", 'هل أنت مستعد لبدء تزكيتك وشفائك؟ تواصل مع الشيخ مباشرة عبر واتساب لاستشارة أولى.');
  ...
  const location = contactSection
    ? localized(meta, 'location', lang)
    : t('Pratique Digitale - Monde entier', 'Digital Practice - Worldwide', 'ممارسة رقمية — في جميع أنحاء العالم');
      ...
      <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <div className="text-center">
            <span className="block text-white font-semibold text-lg">{t('Écrivez-nous sur WhatsApp', 'Message us on WhatsApp', 'راسلنا عبر واتساب')}</span>
            <span className="text-green-200/80 text-sm">{t('Réponse sous 24h', 'Response within 24h', 'الرد خلال ٢٤ ساعة')}</span>
          </div>

...
/** Arabic is labelled in its own script, the way language pickers normally do
 *  it — an Arabic reader scanning the header shouldn't have to find "AR". */
...
const Header = () => {
  ...
  const navItems = [
    ...
    { fr: 'Formations', en: 'Training', ar: 'الدورات', href: '#formations' },
    { fr: 'Livres', en: 'Books', ar: 'الكتب', href: '#books' },
    { fr: 'À propos', en: 'About', ar: 'من نحن', href: '#about' },
    { fr: 'Contact', en: 'Contact', ar: 'تواصل معنا', href: '#contact' },
  ];
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <a href="#home" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:block">
              <span className="force-ltr text-xl font-light tracking-[0.3em] text-transparent bg-clip-text bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600">
                USRATUL AZKAAR
              </span>
              <p className="text-[10px] text-amber-500/60 tracking-widest">
                {t('Pratique spirituelle islamique', 'Islamic Spiritual Practice', 'ممارسة روحانية إسلامية')}
              </p>
            </div>
          </a>
          ...
          <div className="flex items-center gap-4">
            {/* Language Switcher — a segmented control rather than the old
                sliding pill: a slider needs one hard-coded offset per option,
                and those offsets are absolute left values that point the wrong
                way once Arabic flips the document to RTL. */}
            <div
              role="group"
              aria-label={t('Choix de la langue', 'Language selection', 'اختيار اللغة')}
              className="flex items-center h-8 rounded-full bg-black/50 border border-amber-600/30 overflow-hidden"
            >
              {LANGUAGES.map(({ code, label, title }) => (
                <button
                  ...
                  title={title}
                  className={`h-full px-2.5 text-xs font-bold transition-all duration-300 ${
                    lang === code
            ...
            >
              ...
              {t('Prendre RDV', 'Book Now', 'احجز موعدًا')}
            </a>
